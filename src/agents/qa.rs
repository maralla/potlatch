...
**Robustness under failure, load, and scale is first-class coverage too.** A function that is correct under one clean invocation can still lose data or wedge itself in the real world, so actively attack its guarantees — always in the testing environment, and always leave it running afterwards (restart whatever you killed):
- **Destructive testing:** kill the service process (`kill -9`) at varying points — before, during, and after an observed write, in the middle of a multi-step journey — then restart it the way the environment runs it and inspect the aftermath externally: a status stuck in "processing", a write that reported success but is gone, partially written data that can no longer be read, updated, or deleted (unrecoverable partial state), or an index/queue that silently dropped entries is a finding even though every pre-kill step was green. Vary the kill timing and target instead of repeating one point — random kills across a journey surface what a single well-chosen moment misses.
- **Stress testing:** hammer the function to find what only breaks under load — bursts of parallel invocations of the same operation, mixed parallel operations, rapid repeated calls, sustained traffic — and watch for errors absent in single runs, corrupted or interleaved results, leaked resources (connections, temp files, threads), or throughput that collapses rather than degrades.
- **Large data:** exercise functions at and beyond expected scale — oversized single inputs, bulk operations with many entries, large result sets — and verify completeness end-to-end: nothing silently truncated, every entry still retrievable, pagination/filtering intact. Weaknesses that only appear at scale (timeouts, memory exhaustion, silent drops) are findings.
...
- **Read/write/run** your test scripts: `{test_scripts_dir}/`
  Write Python scripts here via `write` (`outside_cwd: true`), **named by the function they test** (e.g. `{test_scripts_dir}/test_search.py`, `{test_scripts_dir}/test_doc_lifecycle.py`), not by issue number or commit SHA. A single script covers all cases for one function — happy path, edge cases, error handling, and regression scenarios as test functions within it. Update existing scripts with `edit` (`outside_cwd: true`) for targeted changes. Run them yourself via `shell` with `outside_cwd: true`: `python3 {test_scripts_dir}/test_search.py`. The harness does not load, save, or run test scripts — you do.
...
mod tests {
    ...
    fn build_function_index_lists_headings_and_scripts() {
        ...
        let index = build_function_index(
            ...
            &[
                "test_search.py".to_string(),
                "test_doc_lifecycle.py".to_string(),
            ],
        );
        ...
        assert!(index.contains("- `test_search.py`"));

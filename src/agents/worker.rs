...
11. If the feedback cannot be resolved without additional human input (for example ambiguous requirements, out-of-scope requests, or missing information), report that clearly and identify the needed input. The same applies when the reviewer states the merge request should be closed or the change is not needed (for example "this MR is invalid, revert all and close it"): do NOT write code — hand off `cannot_resolve` with a reason quoting what the reviewer said.
...
mod tests {
    ...
    fn issue_observation(iid: u64, labels: &[&str]) -> IssueObservation {
        IssueObservation {
            iid,
            title: format!("Reduce the retry delay for issue {iid}"),
    ...
    fn implementation_prepares_the_branch_invokes_the_model_then_opens_the_merge_request() {
        ...
        assert_eq!(
            ...
            strings(&[
                ...
                "stage_all()",
                "has_staged_changes()",
                "commit(Reduce the retry delay\n\nRefs #7)",
                "has_diff_against(main)",
                "push_branch(issue-7)",
                "create_merge_request(issue-7,main,Reduce the retry delay,Closes #7\n\nReduced the retry delay at 30s.)",
                "add_mr_scope_label(12)",
                "require_save_session_with_summary(12,Reduced the retry delay at 30s.)",
            ])
        );
    ...
    fn feedback_updates_metadata_before_touching_the_branch() {
        let resolution = FeedbackResolution {
            mr_title: Some("Reduce the retry delay to 30s".to_string()),
        ...
        assert_eq!(
            &trace[..2],
            &[
                "update_mr_metadata(Reduce the retry delay to 30s|Closes #7)",
                "fetch_branches",
            ]
        );
        assert_eq!(
            port.metadata,
            vec![(
                "Reduce the retry delay to 30s".to_string(),
                "Closes #7".to_string(),
            )]
        );
    }
    ...
    fn feedback_treats_gitlab_reported_conflicts_as_unresolved() {
        let mut input = feedback_input(&["d1"], addressed("Reduced the retry delay"));
        input.requires_conflict_resolution = true;
        let mut port = FakeFeedbackPort::new()
    ...
    fn feedback_without_branch_changes_replies_without_resolving() {
        ...
        let mut port = FakeFeedbackPort::new()
            .with_surface_after(feedback_surface("Reduce the retry delay to 30s", false));
        let (result, trace) = run_feedback(&mut port, feedback_input(&["d1"], resolution));

...
- **QA Agent**: Monitors the configured branches for new commits and runs regression testing and security analysis against the running system
...
Potlatch clones the repository into per-agent directories (`<project>-<agent-id>`, e.g. `my-project-worker-0`) that the AI agent can fully access and modify — there is no interactive confirmation. Only use Potlatch with repositories you trust.
...
[agent.worker]
model = "acp://potlatch/deepseek-v4-flash"   # acp://<vendor>/<model_name> — <vendor> selects the [acp.*] section
...
- **`model`** — `acp://<vendor>/<model>` URI. The `<vendor>` part selects the `[acp.<vendor>]` section; `<model>` is the model name passed to the backend.

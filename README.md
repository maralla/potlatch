...
5. **Chrome or Chromium** - The search agent exposes `search` and `web_fetch`, opens the system browser visibly, and reuses `~/.potlatch/search-chrome-profile` so cookies and consent state persist. Both tools return only Defuddle Markdown without a JSON wrapper: `search` converts the rendered Google results page, while `web_fetch` converts the requested HTTP(S) page. Set `BREEZE_SEARCH_PROFILE` to use another non-default profile directory.
